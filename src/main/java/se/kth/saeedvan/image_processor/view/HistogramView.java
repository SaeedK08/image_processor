 ...
 * A customized JavaFX component, extending VBox, displaying a histogram over
 frequencies
 * per intensities. Lines are displayed for colors red, green and blue
 respectively.
 */
public class HistogramView extends VBox {
     ...
     * Set, and display, values for this histogram chart. The values should be
     presented in an
     * int[256][3] matrix where the first index, 0..255, represents intensity, and
     the second index
     * represents respectively red (0), green (1) and blue (2).
     * The value at [intensity][color] itself represents the number of pixels in an
     image with that
    ...
    public void updateView(int[][] histogramValues) {
        ...
        // add new data
        for (int i = 0; i < 256; i++) {
            seriesRed.getData().add(new XYChart.Data<>(i, histogramValues[i]
                    [0])); // red
            seriesGreen.getData().add(new XYChart.Data<>(i, histogramValues[i]
                    [1])); // green
            seriesBlue.getData().add(new XYChart.Data<>(i, histogramValues[i][2]));
        // blue
        }
        // styling
